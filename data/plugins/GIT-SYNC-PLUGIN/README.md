...
1. New feature: Users can customize the path of the 'assets/' resource file synchronized to the git repository
2. New features: Users can pull down the menu, select Update local resource files (to restore the issue where local resources cannot be displayed normally due to configuring the 'assets/' resource file path), and refresh the work tree (to restore the problem that the ids of some newly added notes cannot be found after data synchronization).
3. BUG Fix: Fixed the issue that some users reported after the last update when synchronizing data prompted: "Please initialize the remote repository/Check if the token configuration in the Settings interface is correct/check the network connection.
4. Optimization function: Optimize global exception handling and log files
...
+ **🚧 Function menu**
  ...
  + 🚀 refresh/update data:
    ...
    + 👉 refresh the working tree: if after synchronization, the newly added local file shows the message "ID not found!" The problem can be solved by refreshing the work tree
    + 👉 update resource path: If configuring the 'assets/' resource file path causes local resources to fail to display properly, you can restore it by pressing this button
...
+ **🚧 Settings screen**
  ...
  + 🚀 resource file path (must end with [assets/]) : After the user configures, the 'assets/' of the resource file link in the files synchronized to the platform can be replaced with a custom path prefix
